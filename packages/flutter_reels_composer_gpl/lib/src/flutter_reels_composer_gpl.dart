...
import 'package:flutter_reels_composer_export_gpl/flutter_reels_composer_export_gpl.dart';
...
/// GPL batteries-included entry point using the x264-enabled FFmpeg build.
///
/// Do not import this package together with `flutter_reels_composer`, whose
/// default facade bundles the mutually exclusive LGPL FFmpeg build.
class FlutterReelsComposerGpl {
  FlutterReelsComposerGpl._();
  ...
  static Future<ComposerResult?> open(
    ...
    final exporter = resolved.exporter ?? FfmpegGplExportPort();
    ...
    try {
      ...
      try {
        ...
        debugPrint('FlutterReelsComposerGpl: effect pack load failed: $error');
      ...
      if (templateCatalog.templates.isEmpty) {
        try {
          ...
          debugPrint(
            'FlutterReelsComposerGpl: template catalog load failed: $error',
          );
          resolved.onEvent?.call(
            ComposerAnalyticsEvent(
